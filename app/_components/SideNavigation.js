		...
		<nav className="border-primary-900 border-r">
			<ul className="flex h-full flex-col gap-2 text-lg">
				{navLinks.map((link) => (
					<li key={link.name}>
						<Link
							className={`hover:bg-primary-900 hover:text-primary-100 text-primary-200 flex items-center gap-4 px-5 py-3 font-semibold transition-colors ${pathName === link.href && "bg-primary-900 text-primary-100"}`}
							href={link.href}
						>
							{link.icon}
							<span>{link.name}</span>
